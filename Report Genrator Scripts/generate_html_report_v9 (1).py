...
def assess_performance(student_data, questions, metadata):
        ...
        if q_id in questions:
            ...
            answers[q_id] = {
                ...
                'correctness': answer.get('correctness',
                                          'correct' if student_answer == correct_answer else 'incorrect' if student_answer != 'skipped' else 'skipped')
            }
    ...
    return {
        'summary': {'total_questions': total_questions, 'attempted': attempted, 'correct': correct,
        ...
        for topic_range, topic_name in data['topics'].items():
            if '-' in topic_range:
                try:
                    ...
                    print(
                        f"Error: Failed to parse topic_range '{topic_range}' in section '{section}' for topic '{topic_name}'. Error: {e}. Skipping.")
                    continue
            else:
                try:
...
def generate_html_report(student_data, questions, metadata, output_dir='reports'):
    ...
    if overall_accuracy < 40:
        tips.append(
            ("Overall Performance: Your score is below 40%. Focus on consistent daily practice across all sections.",
             "critical"))
        ...
        tips.append(
            ("Overall Performance: Excellent work with over 70%! Maintain momentum and refine skills.", "maintenance"))

    if any(area[0] == 'Mathematics' for area in weak_areas):
        tips.append(
            ("Mathematics: Practice 15 problems daily on weak topics like Trigonometry or Calculus.", "critical"))
    ...
    if skipped > total_questions * 0.3:
        tips.append(
            ("Time Management: You skipped over 30% of questions. Practice mock tests to improve pace.", "moderate"))
    ...
    for section, topics in topic_stats.items():
        ...
        topic_charts[section] = {'labels': topic_labels, 'data': topic_data, 'color': color_map.get(section, '#FF9800'),
                                 'stats': topic_stats_list}
    ...
    radar_strengths = [stats['accuracy'] if any(
        (section, topic, s) in strength_areas for topic, s in topic_stats[section].items()) else 0 for section, stats in
                       section_stats.items()]
    radar_weaknesses = [
        stats['accuracy'] if any((section, topic, s) in weak_areas for topic, s in topic_stats[section].items()) else 0
    ...
    quotes_list = "".join([f'<p class="quote {"featured-quote" if i == 0 else ""}">{quote}</p>' for i, quote in
                           enumerate(selected_quotes)])

    base_name = os.path.basename(json_file)
    student_id_parts = base_name.split('_')
    student_id = student_id_parts[2] if len(student_id_parts) > 2 and student_id_parts[0] == 'student' and \
                                        student_id_parts[1] == 'answers' else student_data.get('student_id', 'unknown')
    ...
    <head>
        ...
        <style>
            ...
            .strength-weakness-container {{
                ...
                margin-bottom: 30px; /* Add space between charts */
            }}
            .strength-weakness-wrapper {{
                display: flex;
                flex-direction: column;
                gap: 50px; /* Space between charts */
                height: 900px; /* Default height for smaller screens */
            }}
            /* Adjust heights for larger screens */
            @media (min-width: 768px) {{
                .strength-weakness-wrapper {{
                    height: 1400px; /* Increased to prevent overlap */
                }}
                .strength-weakness-container {{
                    height: 300px; /* Consistent size for larger screens */
                }}
            }}
            @media (min-width: 1440px) {{
                .strength-weakness-wrapper {{
                    height: 1600px; /* Further increase for very large screens */
                }}
                .strength-weakness-container {{
                    height: 350px; /* Slightly larger for high-res screens */
                }}
            }}
    ...
    <body>
        ...
        for section in topic_charts.keys()
            ...
            <div class="section tips-section" style="margin-top: 80px;">
        ...
        for section in topic_charts.keys()
            ...
            function downloadPDF() {{
                ...
                const opt = {{
                    margin: 10,
                    filename: 'MUET_Entry_Test_Report_{student_id}.pdf',
